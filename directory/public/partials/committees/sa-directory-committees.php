...
if (class_exists('SaDirectoryAPI')) {
	...
	$activeAside = "committees";

	$directory_seo_title = returnCommitteeSeoDetails($state, $category)['title'] ?? '';
	$directory_seo_desc = returnCommitteeSeoDetails($state, $category)['description'] ?? '';

	$checkProMember = checkProMember($state);


	$data = $prodApi->get_directories_data("api/committees/" . $state . "/" . $category . "/" . $year);
									...
									<div class="states">
										...
										<select name="states" id="states" class="large gfield_select select2-hidden-accessible custom-select" onchange="handleNavStateChange(event)">
											<?php foreach ($siteStates as $key => $sstate) :
												$stateUrl = site_url('directory/committees/' . strtolower($key) . '/' . $category . '/' . $year);
											?>
									...
									?>
										<div class="dir-committee--items" id="dirCommitteeItems" data-empty="Currently, there are no committees listed for this legislative session.">
											<?php foreach ($data->data as $committee) :
												$slug = $committee->id ?? "";
												$urlLink = site_url('/pro/directories/committee/' . strtolower($state) . '/' . $category . '/' . $year . '/' . $slug);
												$parties = [];
